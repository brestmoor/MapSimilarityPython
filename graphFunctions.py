...
def avg_short_distances_between_train_stations_and_city_center(place):
    network_graph = ox.graph_from_place(place, network_type='drive', custom_filter='["highway"~"motorway|trunk|primary|secondary|tertiary|residential"]')
...
def longest_cycleway_network(place):
    ...
    try:
        graphs = [ox.graph_from_place(place, custom_filter=network_filter, retain_all=True) for network_filter in
                other_cycleways + custom_filters]
